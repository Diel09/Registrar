...
const getRequest = async () => {
  try {
    const response = await axios.get('/get-Ready'); // Replace with your actual endpoint
...
const approveConfirmation = (confirmed) => {
    if (confirmed) {
        axios.post(`/doneStatus`, {id: dtls.value.id}).then(() => {
            showApprove.value = true;
            getRequest();
                        ...
                        <div class="relative w-full max-w-6xl max-h-full">
                            ...
                            <div class="p-6 relative bg-white rounded-lg shadow dark:bg-gray-700">
                                ...
                                <div class="flex justify-end">        
                                    <button type="button" @click="doneStatus()" class="mt-3 text-white bg-green-800 hover:bg-green-600 focus:outline-none focus:ring-4 focus:ring-gray-300 font-medium rounded-full text-sm px-5 py-2.5 mr-2 mb-2 dark:bg-blue-800 dark:hover:bg-blue-600 dark:focus:ring-blue-500 dark:border-gray-700">Done</button>                                     
