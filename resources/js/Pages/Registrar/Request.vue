...
const deleteRequest = (id) => {
            if(confirm('Are you sure?')){
                axios.post('/deleteRequest',{id}).then(({data})=>{
                    if(data){
                        alert('success');
                            ...
                            <tr v-for="(item, index) in docReq" :key="id" class="grid grid-cols-5 border-b-2 border-zinc-100 dark:border-zinc-100">
                                ...
                                <td class="px-3 py-4">                                               
                                    <div class="flex items-center justify-center" >
                                        <label @click="deleteRequest(item.id)" class="flex justify-center focus:outline-none max-md:w-11 text-white bg-red-800 hover:bg-red-700 focus:ring-4 focus:ring-green-300 font-medium rounded-lg text-sm px-5 py-2.5 mr-2 dark:bg-blue-600 dark:hover:bg-blue-700 dark:focus:ring-blue-800">
                                            Delete</label> 
